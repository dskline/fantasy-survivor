...
if (!process.env.VITE_SUPABASE_PUBLIC_ANON_KEY) {
  throw new Error("VITE_SUPABASE_PUBLIC_ANON_KEY is not set");
}
...
if (
  ...
  process.env.VITE_SUPABASE_API_URL.indexOf("localhost") === -1
) {
  throw new Error(
    "VITE_SUPABASE_API_URL is not set to localhost. This test suite requires a local Supabase instance."
  );
}
